...
pub mod pallet {
	...
	impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
		fn offchain_worker(block_number: T::BlockNumber) {
			...
			let call = Call::extrinsic { number: value };
            ...
            // read last block storage
	        let key = Self::derived_key(block_number - 1u32.into());
	        log::info!("local storage key is : {:?}", str::from_utf8(&key).unwrap_or("error key read"));
	...
	impl<T: Config> ValidateUnsigned for Pallet<T> {
		...
		fn validate_unsigned(_source: TransactionSource, call: &Self::Call) -> TransactionValidity {
			if let Call::extrinsic { number: _ } = call {
